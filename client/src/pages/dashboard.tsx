...
export default function Dashboard() {
  ...
  const saveEdits = async () => {
    try {
      // Save all edited products - parse price strings back to numbers
      const promises = Object.entries(editedProducts).map(([id, product]) => {
        ...
        return updateProductMutation.mutateAsync({ id, product: prodCopy });
      });
    ...
    if (newProduct.name.trim() && newProduct.category.trim() && newProduct.price) {
      try {
        await createProductMutation.mutateAsync({
          ...
          price: parsePriceString(newProduct.price),
        });
                  ...
                  <div className="flex items-center gap-3">
                    {isEditMode ? (
                      <>
                        ...
                        <Button onClick={saveEdits} className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-lg shadow-primary/20" data-testid="button-save-edits">
                            ...
                            <TableRow key={product._id} className="border-white/10 hover:bg-white/[0.02] transition-colors group">
                              ...
                              <TableCell className="text-right">
                                {isEditing ? (
                                  ...
                                  <span className="font-mono">{product.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                )}
                              </TableCell>
                              {isAdmin && !isEditMode && (
                                <TableCell className="text-right">
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="bg-card border-white/10 text-foreground">
                                      ...
                                      <DropdownMenuItem className="text-destructive gap-2 focus:bg-destructive/20 focus:text-destructive" onClick={() => handleDelete(product._id)}>
